...
--products_id -> U000001
--orders_id -> U000001
--Create sequence เป็นการ generate เลข id ให้เพิ่มทีละ 1 (sequence nextval โค๊ด sql)
...
CREATE TABLE "users" (             /*CONCAT เพิ่มตัวอักษร*/                     /*:: แปลประเภทตัวแปรใน sql*/
  "id" VARCHAR(7) PRIMARY KEY DEFAULT CONCAT('U', LPAD(NEXTVAL('users_id_seq')::TEXT, 6, '0')), /*คำสั่ง gen id u000001*/
...
-- open auth
CREATE TABLE "oauth" (  /*UNIQUE ห้ามค่าซ้ำกัน*/
...
CREATE TABLE "roles" (
  "id" SERIAL PRIMARY KEY, /*Serial เพื่อให้ id เพิ่ม auto int จะไม่เพิ่ม auto*/
...
CREATE TABLE "orders" (
  ...
  "transfer_slip" jsonb,
  "status" order_status NOT NULL, /*order_status ดึงจาก enum ด้านบน*/
  "created_at" TIMESTAMP NOT NULL DEFAULT now(),
  "updated_at" TIMESTAMP NOT NULL DEFAULT now()
);
